<!-- <template>
    ...
    <div>
        ...
        <div class="flex flex-grow flex-col overflow-y-auto bg-white pt-5">
          ...
          <div class="mt-5 flex flex-1 flex-col">
            <nav class="flex-1 space-y-1 px-2 pb-4">
              <a v-for="item in navigation" :key="item.name" :href="item.href" :class="[item.current ? 'bg-slate-300 text-black' : 'text-indigo-100 hover:bg-emerald-400', 'group flex items-center px-2 py-2 text-sm font-medium ']">
                <component :is="item.icon" class="mr-3 h-6 w-6 flex-shrink-0 text-indigo-800" aria-hidden="true" />
                <!-- {{ item.name }} -->
                <NuxtLink :to="item.path" class="n-link-base" >{{ item.name }}</NuxtLink>
      ...
      <div class="flex flex-1 flex-col md:pl-64">
        <div class="sticky top-0 z-10 flex h-16 flex-shrink-0 bg-white ">
          <div class="mx-auto flex justify-start max-w-5xl px-4 sm:px-6 md:px-8">
            <img src="@/assets/LightRay_logo.jpg" alt="">
             </div>
            ...
            <Bars3BottomLeftIcon class="h-6 w-6" aria-hidden="true" />
                ...
                <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                  <MenuItems class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <MenuItem v-for="item in navigation" :key="item.name" :href="item.href" v-slot="{ active }">
                      <a :to="item.path" :class="[active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700']">{{ item.name }}</a>
                    </MenuItem>
                  </MenuItems>
                </transition>
              </Menu>
  ...
  const navigation = [
    { name: 'Home', href: '#', path:"/" , icon: HomeIcon, current: true },
    // { name: 'WW - Quarters', href: '#', path:"/ww/quarters" , icon: HomeIcon, current: true },
    // { name: 'WW - Filer Lookup', href: '#', path:"/ww/filer_lookup" , icon: HomeIcon, current: true },
    { name: 'SetCookies', href: '#', path:"/setCookies" , icon: HomeIcon, current: true },
    { name: 'About', href: '#', path:"/" , icon: HomeIcon, current: true },
    // { name: 'WW - Holdings', href: '#', path:"/ww/holdings" , icon: HomeIcon, current: true },
    // { name: 'WW - Holders', href: '#', path:"/ww/holders" , icon: HomeIcon, current: true },
    // { name: 'WW - Stock Lookup', href: '#', path:"/ww/stock_lookup" , icon: HomeIcon, current: true },
  ]
